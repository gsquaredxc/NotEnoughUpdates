...
public class APIManager {
    ...
    public boolean isVanillaItem(String internalname) {
        ...
        if (manager.getItemInformation().containsKey(vanillaName)) {
            ...
            if (item.getJson() != null && item.getJson().has("vanilla") && item.getJson().get("vanilla").getAsBoolean())
                return true;
        }
    ...
    public CraftInfo getCraftCost(String internalname, int depth) {
        if (craftCost.containsKey(internalname)) {
            ...
            IItem iitem = manager.getItemInformation().get(internalname);
            if (iitem != null) {
                JsonObject item = iitem.getJson();
                if (item.has("recipe")) {
                    float craftPrice = 0;
